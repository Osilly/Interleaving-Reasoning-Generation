...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        for _, (questions, images, conversations, question_ids, annotations) in tqdm(
            ...
            for question, question_id, answer, annotation in zip(
                questions, question_ids, answers, annotations
            ):
                if ds_name in [
                    "vqav2_val",
                    "vqav2_testdev",
        ...
        if torch.distributed.get_rank() == 0:
            ...
            if ds_collections[ds_name]["metric"] == "vqa_score":
                ...
                annotation = json.load(
                    open(ds_collections[ds_name]["annotation"], "r")
                )["annotations"]
                question_id2answers = {}
                ...
                print(
                    "python eval/vqa/infographicsvqa_eval.py -g "
                    + ds_collections[ds_name]["annotation"]
                    + " -s "
                    + results_file
                )
                os.system(
                ...
                if "gqa" in ds_name:
                    ...
                    print(
                        "python eval/vlm/eval/vqa/convert_gqa_for_eval.py --src "
                        + results_file
                        + " --dst "
                        + dst_file
                    )
                    ...
                    os.system(
                        python_path
                        + " eval/vlm/eval/vqa/convert_gqa_for_eval.py --src "
                        + results_file
                        + " --dst "
                        + dst_file
                    )
                    ...
                    print(command)
                    accuracy = subprocess.check_output(
