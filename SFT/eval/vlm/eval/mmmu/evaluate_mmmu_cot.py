...
COT_OPEN_INSTRUCTION = "Question: {question}"
...
class MMMUDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, split):
        ...
        for subject in tqdm(CAT_SHORT2LONG.values()):
            sub_dataset = load_dataset(
                ...
                subject,
                split=split,
                cache_dir=os.path.join(os.getcwd(), "eval/vlm/data/MMMU/"),
            )
    ...
    def __getitem__(self, idx):
        ...
        if len(choice_txt) > 0:
            question = COT_MC_INSTRUCTION_V2.format(
                question=question.strip(), options=choice_txt.strip()
            )
        else:
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        for _, (questions, images, conversation, answers, data_ids, options) in tqdm(
            ...
            for question, pred, answer, data_id in zip(
                questions, preds, answers, data_ids
            ):
                cur_output = {
                    "question": question,
                    "answer": pred,
