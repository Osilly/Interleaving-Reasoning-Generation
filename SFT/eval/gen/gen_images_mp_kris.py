...
def editing_image_with_think(
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        tmp_past_key_values = gen_model.forward_cache_update_text(
            tmp_past_key_values, **tmp_generation_input
        )

    tmp_generation_input = gen_model.prepare_start_tokens(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate images using CausalFusion model."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    for idx in range(start, end):
        ...
        if isinstance(metadata["ori_img"], str):
            images.append(
                pil_img2rgb(
                    Image.open(
                        os.path.join(image_path, metadata["type"], metadata["ori_img"])
                    )
                )
            )
        else:
            for img_path in metadata["ori_img"]:
                images.append(
                    pil_img2rgb(
                        Image.open(os.path.join(image_path, metadata["type"], img_path))
                    )
                )
