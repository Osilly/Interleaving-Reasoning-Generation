    ...
    def prepare_prompt(self, image_links: List = [], text_prompt: str = ""):
        ...
        for img_link in image_links:
            if type(img_link) == str:
                image_links_base64.append(
                    convert_image_to_base64(
                        process_image(megfile.smart_open(img_link, "rb"))
                    )
                )
            else:
                image_links_base64.append(
                    convert_image_to_base64(process_image(img_link))
                )
        ...
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "image", "image": img_link}
                    for img_link in image_links_base64
                ]
