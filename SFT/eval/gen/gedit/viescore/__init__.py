...
class VIEScore:
    def __init__(
        self, backbone="gpt4o", task="t2i", key_path=None, azure_endpoint=""
    ) -> None:
        self.task = task
        ...
        if self.backbone_name == "gpt4o":
            ...
            self.model = GPT4o(
                key_path, model_name="gpt-4.1-2025-04-14", azure_endpoint=azure_endpoint
            )
        ...
        if self.task == "t2i":
            ...
            self.PQ_prompt = "\n".join(
                [self.context, vie_prompts._prompts_0shot_rule_PQ]
            )
        elif self.task == "tie":
            self.SC_prompt = "\n".join(
                [
            ...
            self.PQ_prompt = "\n".join(
                [self.context, vie_prompts._prompts_0shot_t2v_rule_PQ]
            )
    ...
    def evaluate(
        ...
        if self.task == "tie":
            PQ_prompt_final = self.model.prepare_prompt(
                image_prompts[-1], self.PQ_prompt
            )
        else:
        ...
        while SC_dict is False or PQ_dict is False:
            ...
            SC_dict = mllm_output_to_dict(
                result_SC, give_up_parsing=guess_if_cannot_parse
            )
            PQ_dict = mllm_output_to_dict(
                result_PQ, give_up_parsing=guess_if_cannot_parse
            )
