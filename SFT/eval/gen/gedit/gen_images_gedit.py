...
def setup_models(model_path, device=0):
    ...
    # Load LLM
    llm_config = Qwen2Config.from_pretrained(
    ...
    if use_think:
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            tmp_generation_input = model.prepare_start_tokens(
                newlens, new_rope, new_token_ids
            )
...
def process_dataset(
    ...
    for data_idx in tqdm(idx_list):
        ...
        save_path_fullset_source_image = (
            f"{output_dir}/fullset/{task_type}/{instruction_language}/{key}_SRCIMG.png"
        )
        save_path_fullset = (
            f"{output_dir}/fullset/{task_type}/{instruction_language}/{key}.png"
        )
        ...
        if os.path.exists(save_path_fullset_source_image) and os.path.exists(
            save_path_fullset
        ):
...
def main():
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to bagel model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cfg_text_scale", type=float, default=4.0, help="Text CFG scale"
    )
    parser.add_argument(
        "--cfg_img_scale", type=float, default=1.5, help="Image CFG scale"
    )
    parser.add_argument(
        "--cfg_type",
        type=str,
    ...
    parser.add_argument(
        "--shard_id", type=int, default=0, help="ID of the current shard (0-based)"
    )
    parser.add_argument(
        "--total_shards", type=int, default=1, help="Total number of shards"
    )
    parser.add_argument(
        "--use_think", action="store_true", help="Whether enable thinking"
    )
    ...
    model, vae_model, tokenizer, new_token_ids, vae_transform, vit_transform = (
        setup_models(args.model_path, args.device)
    )
