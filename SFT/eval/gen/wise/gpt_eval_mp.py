...
def parse_arguments():
    ...
    parser.add_argument(
        "--json_path", required=True, help="Path to the prompts JSON file"
    )
    parser.add_argument(
        "--image_dir", required=True, help="Path to the image directory"
    )
    parser.add_argument(
...
def extract_scores(evaluation_text: str) -> Dict[str, float]:
    score_pattern = (
        r"\*{0,2}(Consistency|Realism|Aesthetic Quality)\*{0,2}\s*[:：]?\s*(\d)"
    )
...
def evaluate_image(prompt_data: Dict, image_path: str, config: Dict) -> Dict[str, Any]:
    try:
        ...
        return {"evaluation": evaluation_text, **scores}
...
if __name__ == "__main__":
    ...
    processed_ids = set()
    if os.path.exists(
        os.path.join(config["output_dir"], config["result_files"]["full"])
    ):
        with open(
            os.path.join(config["output_dir"], config["result_files"]["full"]),
            "r",
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        futures = [
            executor.submit(process_prompt, prompt_id, prompt_data, config)
