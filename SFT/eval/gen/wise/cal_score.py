...
def cal_culture(file_path):
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if 9.9 in [data["consistency"], data["realism"], data["aesthetic_quality"]]:
                has_9_9 = True
            wiscore = calculate_wiscore(
                data["consistency"], data["realism"], data["aesthetic_quality"]
            )
    ...
    score = {"total": total_score, "average": avg_score}
...
def cal_space_time(file_path):
    ...
    avg_scores = {
        category: sum(scores) / (len(scores) * 2) if len(scores) > 0 else 0
        for category, scores in categories.items()
    }

    scores = {"total": total_scores, "average": avg_scores}

    print(
        f"  Time - Total: {scores['total'].get('Time', 0):.2f}, Average: {scores['average'].get('Time', 0):.2f}"
    )
    print(
        f"  Space - Total: {scores['total'].get('Space', 0):.2f}, Average: {scores['average'].get('Space', 0):.2f}"
    )
...
def cal_science(file_path):
    ...
    for category in ["Biology", "Physics", "Chemistry"]:
        print(
            f"  {category} - Total: {scores['total'].get(category, 0):.2f}, Average: {scores['average'].get(category, 0):.2f}"
        )
...
if __name__ == "__main__":
    ...
    scores = cal_science(os.path.join(args.output_dir, "natural_science_scores.jsonl"))
    avg_score.update(scores)
