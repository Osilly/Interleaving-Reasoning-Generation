    ...
    if os.path.exists(jsonl_path):
        ...
        for key, merged_result in key_results.items():
            ...
            if all_metrics_complete:
                ...
                logging.info(
                    f"Incomplete result for {key}: missing {', '.join(incomplete_metrics)}"
                )
...
def extract_dual_scores(response):
    ...
    if data:
        return {
            "instruction_score": (
                int(data.get("instruction_score", 0))
                if data.get("instruction_score") is not None
                else None
            ),
            "knowledge_score": (
                int(data.get("knowledge_score", 0))
                if data.get("knowledge_score") is not None
                else None
            ),
...
def evaluate_images(model, category, image_id, metrics=None):
    ...
    if not entry:
        logging.warning(
            f"Image ID {image_id} not in annotations for category {category}"
        )
        return results
    ...
    for metric in metrics:
        if metric == "consistency":
            ...
            results.update(
                {"consistency_score": score, "consistency_reasoning": reasoning}
            )

        elif metric == "dual_score":
            prompt = prompt_dual_evaluation.format(
                instruct=instruct, explanation=explain
            )
            resp = evaluate_with_gpt(prompt, orig_b64, edit_b64)
...
def process_image_eval(
    model, category, image_id, metrics, annotations, output_jsonl_path
):
...
def run_evaluation(models=None, categories=None, metrics=None):
    ...
    for model in models:
        for category in tqdm(categories, desc=f"Evaluating {model}"):
            ...
            key_missing_metrics, fully_completed_keys = (
                load_processed_keys_with_missing_metrics(
            ...
            if not to_process:
                logging.info(
                    f"No images to process for {model}/{category}. All {len(fully_completed_keys)} images are fully completed."
                )
            else:
                logging.info(
                    f"Processing {len(to_process)} images for {model}/{category}. {len(fully_completed_keys)} images already completed."
                )
                    ...
                    for fut in tqdm(
                        ...
                        try:
                            ...
                            if result:
                                ...
                                logging.debug(
                                    f"Completed {img_id} with metrics {img_metrics}"
                                )
                        except Exception as e:
                            img_id, img_metrics = futures[fut]
                            logging.error(
                                f"Failed processing {img_id} with metrics {img_metrics}: {e}"
                            )

            try:
                # Collect final results (only complete ones)
                metrics_data = collect_jsonl_to_dict(
                    metrics_jsonl, metrics, expected_keys_map
                )
                with open(metrics_file, "w", encoding="utf-8") as wf:
                    json.dump(metrics_data, wf, ensure_ascii=False, indent=2)
                logging.info(
                    f"Saved {len(metrics_data)} complete results to {metrics_file}"
                )
            except Exception as e:
