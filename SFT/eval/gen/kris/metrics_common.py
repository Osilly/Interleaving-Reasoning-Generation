...
def evaluate_images(model_name, category, image_id, metrics=None):
    ...
    for m in metrics:
        if m == "consistency":
            ...
            prompt = (
                prompt_abnormal_instruction_following.format(
                    instruct=instr, explanation=expl
                )
                if "abnormality_correction" in category
                else prompt_instruction_following.format(instruct=instr)
            )
            resp = evaluate_with_gpt(prompt, orig_b64, edit_b64)
            score, reasoning = extract_instruction_score_and_reason(resp)
            results.update(
                {"instruction_score": score, "instruction_reasoning": reasoning}
            )
