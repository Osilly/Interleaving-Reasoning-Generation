...
def evaluate_temporal_with_gpt(
    prompt, reference_base64_list, predicted_base64, frame_numbers, pred_frame_num
):
    """
...
def evaluate_temporal_images(model_name, category, image_id, metrics=None):
    ...
    for metric in metrics:
        if metric == "consistency":
            prompt = prompt_consist_temporal.format(
                N=pred_frame_num, instruct=instruction
            )
            resp = evaluate_temporal_with_gpt(
                prompt, ref_b64s, pred_b64, frame_numbers, pred_frame_num
            )
            ...
            prompt = prompt_instruction_temporal.format(
                N=pred_frame_num, instruct=instruction
            )
            resp = evaluate_temporal_with_gpt(
                prompt, ref_b64s, pred_b64, frame_numbers, pred_frame_num
...
def process_temporal_image_eval(
