...
def eval_vanilla(item, input_dir, output_dir, **kwargs):
    ...
    if category in ["temporal_reasoning", "causal_reasoning"]:
        ...
        if "reference_img" in item and not pd.isna(item["reasoning_img"]):
            ...
            prompt_rea = prompt_reasoning_w_input.format(
                instruct=instruct, reference=reference
            )
        else:
        ...
        if "reference_img" in item and not pd.isna(item["reference_img"]):
            ...
            judge_rea_require_img = True
            reference = item["reference"]
            prompt_rea = prompt_spatial_ref_w_input.format(
                instruct=instruct, reference=reference
            )
        else:
            reference = item["reference"]
            prompt_rea = prompt_spatial_ref.format(
                instruct=instruct, reference=reference
            )
        ...
        if "reference_txt" in item and not pd.isna(item["reference_txt"]):
            ...
            prompt_cons = prompt_logical_cons_ans.format(
                instruct=instruct, reference=reference
            )
            prompt_rea = prompt_logical_txt.format(
                instruct=instruct, reference=reference
            )
...
def calculate_score(row):
    if row["category"] in [
        ...
        if "consistency_free" in row and row["consistency_free"]:
            ...
            score = (
                0.3 * row["ApprConsistency"]
                + 0.5 * row["Reasoning"]
                + 0.2 * row["VisualPlausibility"]
            )
...
def calculate_completion(row):
    if row["category"] in [
        ...
        return (
            1
            if row["ApprConsistency"] == 5
            and row["Reasoning"] == 5
            and row["VisualPlausibility"] == 5
            else 0
        )
    elif row["category"] == "logical_reasoning":
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Output Image Dir, outputs/MODEL_NAME"
    )
    ...
    temporal_final, temporal_comp_rate = (
        df_temporal["score"].mean(),
        df_temporal["complete"].mean(),
    )
    causal_final, causal_comp_rate = (
        df_causal["score"].mean(),
        df_causal["complete"].mean(),
    )
    spatial_final, spatial_comp_rate = (
        df_spatial["score"].mean(),
        df_spatial["complete"].mean(),
    )
    logical_final, logical_comp_rate = (
        df_logical["score"].mean(),
        df_logical["complete"].mean(),
    )
    ...
    temp_rea_avg, temp_cons_avg, temp_qua_avg = (
        df_temporal["Reasoning"].mean(),
        df_temporal["ApprConsistency"].mean(),
        df_temporal["VisualPlausibility"].mean(),
    )
    cau_rea_avg, cau_cons_avg, cau_qua_avg = (
        df_causal["Reasoning"].mean(),
        df_causal["ApprConsistency"].mean(),
        df_causal["VisualPlausibility"].mean(),
    )
    spa_rea_avg, spa_cons_avg, spa_qua_avg = (
        df_spatial["Reasoning"].mean(),
        df_spatial["ApprConsistency"].mean(),
        df_spatial["VisualPlausibility"].mean(),
    )
    logic_rea_avg, logic_cons_avg, logic_qua_avg = (
        df_logical["Reasoning"].mean(),
        df_logical["ApprConsistency"].mean(),
        df_logical["VisualPlausibility"].mean(),
    )
    ...
    final_score = dict(
        ...
        Overall_Reasoning=[
            reasoning_average,
            trans_to_percent(reasoning_average),
            None,
        ],
        Overall_ApprConsistency=[
            img_consist_average,
            trans_to_percent(img_consist_average),
            None,
        ],
        Overall_VisualPlausibility_total=[
            generation_quality,
            trans_to_percent(generation_quality),
            None,
        ],
    ...
    df = pd.DataFrame(
        final_score, index=["Score-Origin", "Score-Percentage", "Accuracy"]
    ).T
