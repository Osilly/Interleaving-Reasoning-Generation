...
def fsdp_ema_update(ema_model, model, decay=0.9999):
    ...
    for ema_handle, new_handle in zip(ema_handles, new_handles):
        if ema_handle.flat_param is not None:
            if new_handle.flat_param.requires_grad:
                # For trainable parameters, use EMA update
                ema_params.append(ema_handle.flat_param.data)
                new_params.append(
                    new_handle.flat_param.data.to(dtype=ema_handle.flat_param.dtype)
                )
            else:
                # For frozen parameters, directly copy current weights to EMA
                frozen_ema_params.append(ema_handle.flat_param.data)
                frozen_new_params.append(
                    new_handle.flat_param.data.to(dtype=ema_handle.flat_param.dtype)
                )

    # Update trainable parameters with EMA
    if ema_params:
    ...
    if frozen_ema_params:
        torch._foreach_copy_(frozen_ema_params, frozen_new_params)
